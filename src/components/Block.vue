  ...
  <div class="group flex w-full rounded"
    ...
    <div class="h-full pl-4 pr-2 text-center cursor-pointer transition-all duration-150 text-neutral-300 flex"
      :class="{
        ...
        'py-1.5': ![BlockType.H1, BlockType.H2, BlockType.H3].includes(block.type as any),
        // 'py-1.5': !new Set([BlockType.H1, BlockType.H2, BlockType.H3]).has(block.type as any),
      }">
      <HoverBar value="<span class='text-neutral-400'><span class='text-white'>Click</span> to delete block</span>">
      ...
      <HoverBar value="<span class='text-neutral-400'><span class='text-white'>Click</span> to add block below</span>">
...
import { BlockComponents, isTextBlock } from '@/utils/types'
import {BlockType } from '@/utils/types'
import type {Block} from '@/utils/types'
import BlockMenu from './BlockMenu.vue'
import HoverBar from './elements/HoverBar.vue'
...
function isContentBlock () {
  return [BlockType.Text, BlockType.Quote, BlockType.H1, BlockType.H2, BlockType.H3].includes(props.block.type as any)
}
...
function getCaretPos () {
  ...
  if (isTextBlock(props.block.type)) {
      ...
      if (selectedNode !== null && selectedNode.childNodes.length > 0) {
        if (selectedNode.childNodes[0] && selectedNode.childNodes[0].textContent && selectedNode.childNodes[0].textContent.length <= 1)
