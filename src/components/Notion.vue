...
import {isTextBlock, availableBlockTypes } from '@/utils/types'
...
document.addEventListener('mouseup', (event:MouseEvent) => {
  ...
  if ((event.clientX < ((editorRect as DOMRect).left || -1)) || (event.clientX > (editorRect?.right || window.innerWidth))) {
    ...
    const block = blockRects.find(child => {
      ...
      return event.clientY > (rect as DOMRect).top && event.clientY < (rect as DOMRect).bottom
    })
    ...
    if (block) {
      ...
      if (event.x < (rect as DOMRect).left) {
